...
from fasttraders.constants import DATETIME_PRINT_FORMAT
...
class TimeRange:
    ...
    def __init__(
        self, starttype: Optional[str] = None,
    ...
    def timerange_str(self) -> str:
        """
        Returns a string representation of the timerange as used by
        parse_timerange.
        Follows the format yyyymmdd-yyyymmdd - leaving out the parts that are
        not set.
        """
        start = ''
        stop = ''
    ...
    def __eq__(self, other):
        ...
        return (
            self.starttype == other.starttype and self.stoptype ==
            other.stoptype
            and self.startts == other.startts and self.stopts ==
            other.stopts)
    ...
    def adjust_start_if_necessary(
        self, timeframe_secs: int, startup_candles: int,
        min_date: datetime
    ) -> None:
        """
        Adjust startts by <startup_candles> candles.
        Applies only if no startup-candles have been available.
        :param timeframe_secs: Timeframe in seconds e.g.
        `timeframe_to_seconds('5m')`
        :param startup_candles: Number of candles to move start-date forward
        :param min_date: Minimum data date loaded. Key kriterium to decide if
        start-time
                         has to be moved
        :return: None (Modifies the object in place)
        """
        if not self.starttype or (
            ...
            # If no startts was defined, or backtest-data starts at the
            # defined backtest-date
            logger.warning(
    ...
    def parse_timerange(cls, text: Optional[str]) -> Self:
        """
        Parse the value of the argument --timerange to determine what is the
        range desired
