...
def decimal_to_precision(
    n, rounding_mode=ROUND, precision=0, counting_mode=DECIMAL_PLACES,
    padding_mode=NO_PADDING
):
    assert precision is not None
    if counting_mode == TICK_SIZE:
        assert (
            isinstance(precision, float) or
            isinstance(precision, decimal.Decimal) or
            isinstance(precision, numbers.Integral) or
    ...
    # all default except decimal.Underflow
    # (raised when a number is rounded to zero)
    ...
    if counting_mode == TICK_SIZE:
        # python modulo with negative numbers behaves different than js/php,
        # so use abs first
        ...
        return decimal_to_precision('{:f}'.format(dec), ROUND, new_precision,
                                    DECIMAL_PLACES, padding_mode)
    ...
    if rounding_mode == ROUND:
        if counting_mode == DECIMAL_PLACES:
            ...
            if q < 0:
                ...
                above = below + sigfig
                precise = '{:f}'.format(
                    min((below, above), key=lambda x: abs(x - dec)))
            else:
