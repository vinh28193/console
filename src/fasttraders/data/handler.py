...
class DataHandler(ABC):
    ...
    @classmethod
    def ohlcv_get_pairs(cls, datadir: Path, timeframe: str,
                        candle_type: CandleType) -> List[str]:
        """
        Returns a list of all pairs with ohlcv data available in this datadir
    ...
    def ohlcv_data_min_max(
        self, pair: str, timeframe: str, candle_type: CandleType) -> Tuple[
        ...
        return df.iloc[0]['date'].to_pydatetime(), df.iloc[-1][
            'date'].to_pydatetime(), len(df)
    ...
    def _ohlcv_load(self, pair: str, timeframe: str,
        ...
        Timerange trimming and dataframe validation happens outside of this
        method.
    ...
    def _trades_load(
        ...
        :param timerange: Timerange to load trades for - currently not
        implemented
    ...
    def rebuild_pair_from_filename(pair: str) -> str:
        ...
        Assumes a asset name of max. 7 length to also support BTC-PERP and
        BTC-PERP:USD names.
        """
        res = re.sub(r'^(([A-Za-z\d]{1,10})|^([A-Za-z\-]{1,6}))(_)', r'\g<1>/',
                     pair, 1)
        res = re.sub('_', ':', res, 1)
        return res

    def ohlcv_load(
        self, pair, timeframe: str,
        candle_type: CandleType, *,
        ...
        if self._check_empty_df(pairdf, pair, timeframe, candle_type,
            ...
            self._check_empty_df(
                pairdf, pair, timeframe, candle_type,
                warn_no_data
            )
            return pairdf

    def _check_empty_df(
        self, pairdf: DataFrame, pair: str, timeframe: str,
        ...
        if pairdf.empty:
            ...
            candle_price_gap = 0
            if (
                candle_type in (CandleType.SPOT, CandleType.FUTURES) and
                not pairdf.empty and
                'close' in pairdf.columns and
                'open' in pairdf.columns
            ):
                # Detect gaps between prior close and open
                _open = pairdf['open']
                _close = pairdf['close'].shift(1)
                gaps = (_open - _close) / _close
                gaps = gaps.dropna()
                if len(gaps):
                    candle_price_gap = max(abs(gaps))
            if candle_price_gap > 0.1:
                logger.info(
                    f"Price jump in {pair}, {timeframe}, {candle_type} "
                    f"between two candles "
                    f"of {candle_price_gap:.2%} detected.")

        return False

    def _validate_pairdata(
        self, pair, pairdata: DataFrame, timeframe: str,
    ...
    def rename_futures_data(
        self, pair: str, new_pair: str, timeframe: str,
        candle_type: CandleType
    ):
        """
        Temporary method to migrate data from old naming to new naming (
        BTC/USDT -> BTC/USDT:USDT)
        Only used for binance to support the binance futures naming unification.
        """

        file_old = self._pair_data_filename(
    ...
    def fix_funding_fee_timeframe(self, ff_timeframe: str):
        """
        Temporary method to migrate data from old funding fee timeframe to
        the correct timeframe
        Applies to bybit and okx, where funding-fee and mark candles have
        different timeframes.
        """
        paircombs = self.ohlcv_get_available_data(
            self._datadir, TradingMode.FUTURES
        )
        funding_rate_combs = [
            f for f in paircombs if
            f[2] == CandleType.FUNDING_RATE and f[1] != ff_timeframe
        ]

        if funding_rate_combs:
            logger.warning(
                f'Migrating {len(funding_rate_combs)} funding fees to correct '
                f'timeframe.'
            )
